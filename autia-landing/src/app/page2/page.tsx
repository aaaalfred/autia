...
export default function AutiaLanding2() {
      ...
      <section id="servicios" className="bg-neutral-50 py-16">
        <div className="mx-auto max-w-7xl px-4">
          <div className="text-center">
            ...
            <p className="mt-2 text-neutral-600">Tecnología moderna para resolver problemas reales.</p>
          </div>

          <div className="mt-12 space-y-16">
            {servicios.map(({ icon: Icon, title, desc, image }, idx) => (
              <div key={idx} className={`grid items-center gap-12 md:grid-cols-2`}>
                <div className={`relative rounded-3xl overflow-hidden ${idx % 2 === 0 ? 'md:order-2' : ''}`}>
                    <Image
                        src={image}
                        alt={title}
                        width={600}
                ...
                <div className={`${idx % 2 === 0 ? 'md:order-1' : ''}`}>
                  ...
                  <p className="mt-4 text-lg text-neutral-600">{desc}</p>
                </div>
              </div>
            ))}
          </div>
